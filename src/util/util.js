...
import { mfsPrefix } from '@/config/env'
...
/**
 * 格式化文件服务器路径
 * @param fileName 文件服服务存储[bucketName-fileName]
 * @returns {string} '/文件服务器前缀/bucketName/fileName'
 */
export const handleMfsPath = (fileName) => {
    if (validatenull(fileName)) {
        console.warn('fileName is null')
        return ''
    }
    let array = fileName.split('-')
    return `/${mfsPrefix}/${array[0]}/${array[1]}`
}
...
//下载
export function handleDown(title, fileName) {
    return request({
        url: `/admin/file/${fileName}`,
        method: 'get',
        responseType: 'blob',
